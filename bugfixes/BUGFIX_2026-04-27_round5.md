...
| **9** | Strength exercise creator UI — collapse from ~5 vertical units per exercise to ~1 row. Drop the Weighted toggle (weight value alone conveys it). Drop column headers. Drop the always-visible "Customize per set" link. Use a colored left edge to indicate supersets. **Apply across every place strength exercises are manually created or edited** — custom plan, quick add session, workout editor, saved library. |
...
## 🎨 Strength exercise creator — single-row redesign

### Bug 9 — The manual exercise creator wastes space and bloats vertical scroll

**Symptom.** The current strength creator (custom plan, quick add session, workout editor) renders each exercise as a 5-row card: superset header bar → name input row → column labels (`SETS · REPS · WEIGHT`) → input values row → `Customize per set ▾` row. A 3-exercise workout fills 1.5 screens of scroll. Empty placeholder cards take up just as much room as filled ones.

There's also a Weighted/Bodyweight pill toggle that's redundant — the weight field already conveys whether the exercise is loaded.

**Root cause.** Three independent UI sources implement the same exercise-row pattern, all on the maximalist side:
- `js/custom-plan.js:1193+` — manual plan creation (the screenshot showed this).
- `js/calendar.js:5829, 6848, 6922` — quick add session strength (`qe-exercise-list`, `qe-exercise-header`).
- `js/workout-editor.js:228+` — workout editing screen.

Plus a shared toggle module: `js/exercise-row-weight-toggle.js` — handles the Weighted/BW pill rendering. To be retired in favor of smart parsing.

**Expected.** A single shared exercise-row component used by all three surfaces. One row per exercise, expandable on tap. Left-edge accent groups supersets. No weighted toggle. No column headers. No always-visible customize link.

---

#### Visual design (refer to mockup `strength_creator_proposed_v2`)

**Single-row collapsed state:**
```
┃ ⋮⋮  Belt Squat              2 × 10 · — lbs    ▾  ×
┃ ⋮⋮  Bench Press             4 × 10 · 215 lbs  ▾  ×
  ⋮⋮  Pull-up                 3 × 8 · BW        ▾  ×
                              [+ Add exercise]
```

- `┃` left-edge accent (3px solid, purple `#534AB7`) groups supersets.
- Group header sits inside the accent: small uppercase pill `Group A · 3 rounds`.
- Drag handle, exercise name, spec line, expand chevron, delete `×`.
- Empty state collapses to a dashed `+ Add exercise` button — no full-card placeholder.

**Expanded state (on row tap or chevron tap):**
```
┃ ⋮⋮  Belt Squat                              ▾  ×
┃     Sets [2]   Reps [10]   Weight [______]
┃     Customize per set ▾
```
- Customize-per-set link is ONLY visible inside the expanded state.
- Tapping any value chip in the collapsed spec also opens the expanded state focused on that field.

---

#### Smart weight parsing — drop the Weighted toggle

| User input | Stored value | Display in collapsed spec |
|---|---|---|
| (empty) | `null` | `— lbs` (muted) |
| `0`, `bw`, `BW`, `bodyweight` | `BW` | `BW` |
| `215`, `215 lbs`, `215lbs` | `215 lbs` | `215 lbs` |
| `+25`, `bw+25`, `BW +25` | `BW +25 lbs` | `BW +25 lbs` |

- Smart parser lives in a shared util — recommend `js/utils/weight-parser.js` — used by the new exercise-row component AND by the live tracker (so logged set weights parse the same way).
- The existing `js/exercise-row-weight-toggle.js` module is **retired**. Delete it after confirming no remaining callers.
- Edge case: assisted-pull-up machines have inverted weight semantics (more weight = easier). **Do not try to overload the BW field with negative numbers.** "Assisted Pull-up" should be a distinct exercise with its own weight semantic (`Assisted -50` displayed; user enters `-50` or `assist 50`; stored as `assisted_lbs: 50`). Mark this as out-of-scope for Bug 9 — flag a follow-up if assisted machines need first-class support.

---

#### Surfaces to update

The same component must replace the existing rendering in all of these:

1. **`js/custom-plan.js`** lines 1193+ — the row built by `_cpRenderExerciseRow` (or equivalent). This was the screenshot.
2. **`js/calendar.js`** lines 5829, 6848, 6922 — the `qe-exercise-list` rendering for quick add session strength. Currently uses a `qe-exercise-header` row of column labels — drop that.
3. **`js/workout-editor.js`** lines 228+ — the edit-existing-workout screen. Has its own copy of `Customize per set ▾` button at line 228. Replace.
4. **`js/ui/saved-library-tab-view.js`** lines 386+ — already reads `supersetGroup` correctly; render path needs to be updated to the new component.

Recommended approach: extract a single `js/ui/strength-exercise-row.js` module exposing `renderRow({ exercise, mode, callbacks })`. Each of the four surfaces calls into it. One source of truth for layout, parsing, and editing behavior.
...
**Visual:**
- A 5-exercise workout fits in ~one mobile screen without scrolling, vs. ~1.5 screens currently.
- Supersets visibly group via the left edge accent.
- Empty state is a single dashed button, not a full empty card.
- No `SETS · REPS · WEIGHT` column header row anywhere in the UI.
- No bottom `A2` pill or any other duplicate superset indicator — the edge accent is the only signal.

**Interaction:**
- Tap a collapsed row → expands inline. Other rows stay collapsed.
- Tap a value (`2`, `10`, `215 lbs`) in collapsed state → expands AND focuses that field.
- Tap the chevron `▾` → expands. Chevron rotates `▴` when expanded.
- Tap outside an expanded row OR tap chevron again → collapses.
- Drag handle still works for reorder + drop-to-superset (existing behavior preserved).
- Delete `×` confirms only when row has data; empty rows delete silently.

**Data:**
- Smart weight parser correctly handles all 4 cases in the table above. Add unit tests.
- No regression in existing supersets — `supersetGroup`/`supersetId` data continues to round-trip correctly through save/load.
- Exercises imported from saved library or shared workouts render in the new component.

**Cross-surface consistency:**
- The exact same row look + behavior in custom plan, quick add, workout editor, saved library.
- Single shared module — when the coder touches the row, they touch one file.

---

#### Where to look (consolidated)

- **New module:** `js/ui/strength-exercise-row.js` (create).
- **New module:** `js/utils/weight-parser.js` (create).
- **Refactor:** `js/custom-plan.js` lines ~1193–1505, `js/calendar.js` lines ~5829–7100 area (qe-strength rendering), `js/workout-editor.js` lines ~225–340.
- **Retire:** `js/exercise-row-weight-toggle.js` (delete after confirming no callers).
- **CSS:** add `.ex-row-*` classes to `style.css`. Old `.ex-row--bw`, `.ex-row--weighted`, `.ex-row--bw-toggleable`, `.cp-step-row`, `.qe-exercise-header` rules are replaced.

---

#### Tests to add

- `tests/weight-parser.test.js` — all four input modes round-trip correctly.
- `tests/exercise-row-component.test.js` — renders consistently in custom-plan / quick-add / workout-editor contexts (use jsdom + a fixture for each surface).
- Manual test checklist: build a superset in each surface, save, reload, open in tracker — confirm grouping survives end-to-end.

---

## Ship plan — 7 commits, each reviewable in <20 min
...
| **F6: Strength exercise creator redesign** | 9 | **Largest commit.** Build shared `strength-exercise-row.js` + `weight-parser.js`. Wire into custom-plan, quick add, workout editor, saved library. Retire `exercise-row-weight-toggle.js`. ~2-3 hours. **Ship before F7 so the confetti has a clean UI to land in.** |
| **F7: Confetti** | 1 | Pure feature, low risk. Ship last so it's the dopamine hit when everything else lands. |
...
- F6: `tests/weight-parser.test.js` — all four input modes (empty, BW, number, BW+N) parse correctly. `tests/exercise-row-component.test.js` — renders consistently across the four surface contexts.
