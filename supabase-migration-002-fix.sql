-- ============================================================================
-- IRONZ — Migration 002 (fixed)
-- Drops old philosophy_modules/exercise_library if they have the wrong schema,
-- then creates all new tables.
-- ============================================================================

-- Drop old tables that may have the wrong schema (JSONB-blob versions)
-- CASCADE handles any dependent objects (indexes, policies)
DROP TABLE IF EXISTS module_version_history CASCADE;
DROP TABLE IF EXISTS philosophy_gaps CASCADE;
...
DROP TABLE IF EXISTS philosophy_modules CASCADE;


-- ── user_data (generic key-value store) ────────────────────────────────────
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_data' AND policyname='Users can view own user_data') THEN
    CREATE POLICY "Users can view own user_data" ON user_data FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_data' AND policyname='Users can insert own user_data') THEN
    CREATE POLICY "Users can insert own user_data" ON user_data FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_data' AND policyname='Users can update own user_data') THEN
    CREATE POLICY "Users can update own user_data" ON user_data FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_data' AND policyname='Users can delete own user_data') THEN
    CREATE POLICY "Users can delete own user_data" ON user_data FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
-- ============================================================================
-- Done! Run seedReferenceData() in the browser console to populate
-- philosophy_modules and exercise_library with data.
-- ============================================================================
